...
- Local/native (`reranker_service": "remote"`): change only `model_name_or_path` (and optionally `batch_size`) in `api/run_configs/default.json`; rerun `./start_hybrid.sh`. No code changes needed.
- Modal/cloud (`reranker_service": "modal"`): set `MODEL_NAME` in `api/solaceai/rag/reranker/modal_deploy/ai2-scholar-qa-reranker.py`, redeploy with `modal deploy ...`, and keep `app_name` consistent between the deploy script and `default.json`.
