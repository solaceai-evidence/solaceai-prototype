...
import { InlinePaperChipWidgetProps, InlinePaperChipWidget } from './InlinePaperChipWidget';
import { EvidenceCard } from './EvidenceCard';
import { Evidence } from './utils';
import { PaperDetails } from 'src/@types/AsyncTaskState';
import { Badge } from '@mui/material';

interface InlinePaperChipWidgetWithEvidenceProps extends InlinePaperChipWidgetProps {
  evidences: Evidence[];
  fullTitle: string;
  id: string;
...
export const InlinePaperChipWidgetWithEvidence: React.FC<InlinePaperChipWidgetWithEvidenceProps> = (
  props
) => {
  ...
  return (
    <>
      ...
      >
        <Badge
          badgeContent={noBadge ? 0 : (paperDetails?.n_citations ?? 0)}
          color="secondary"
          max={99}
          sx={{
            '& .MuiBadge-badge': {
