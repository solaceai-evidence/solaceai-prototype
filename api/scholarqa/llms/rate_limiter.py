...
class RateLimiter:
    ...
    def __init__(
        ...
        self.input_token_usage = (
            []
        )  # List of (timestamp, input_tokens) tuples for input TPM
        self.output_token_usage = (
            []
        )  # List of (timestamp, output_tokens) tuples for output TPM
        ...
        logger.info(
            f"Rate limiter initialized: {self.max_requests_per_minute} req/min, "
    ...
    def acquire(self):
        ...
        if self.max_workers > 1:
            ...
            with self.worker_condition:
                while self.active_workers >= self.max_workers:
                    logger.info(
                        f"Max workers reached ({self.active_workers}/{self.max_workers}), waiting..."
                    )
                ...
                logger.debug(
                    f"Worker acquired. Active workers: {self.active_workers}/{self.max_workers}"
                )
        ...
        with self.lock:
            ...
            logger.debug(
                f"Request acquired. Current usage: {len(self.request_times)}/{self.max_requests_per_minute}"
            )
    ...
    def release(self):
        ...
        if self.max_workers > 1:
            with self.worker_condition:
                self.active_workers -= 1
                logger.debug(
                    f"Worker released. Active workers: {self.active_workers}/{self.max_workers}"
                )
    ...
    def record_token_usage(self, input_tokens: int, output_tokens: int):
        ...
        with self.lock:
            ...
            cutoff_time = current_time - 60
            self.input_token_usage = [
                (t, tokens) for t, tokens in self.input_token_usage if t > cutoff_time
            ]
            self.output_token_usage = [
                (t, tokens) for t, tokens in self.output_token_usage if t > cutoff_time
            ]
    ...
    def check_token_limits(
        self, estimated_input_tokens: int, estimated_output_tokens: int = 0
    ) -> bool:
        """
        ...
        with self.lock:
            ...
            current_input_tokens = sum(
                tokens for t, tokens in self.input_token_usage if t > cutoff_time
            )
            current_output_tokens = sum(
                tokens for t, tokens in self.output_token_usage if t > cutoff_time
            )
            ...
            if not input_ok:
                logger.warning(
                    f"Input token limit would be exceeded: {projected_input}/{self.max_input_tokens_per_minute}"
                )
            if not output_ok:
                logger.warning(
                    f"Output token limit would be exceeded: {projected_output}/{self.max_output_tokens_per_minute}"
                )
    ...
    @contextmanager
    def request_context(
        self, estimated_input_tokens: int = 0, estimated_output_tokens: int = 0
    ):
        """
        ...
        try:
            ...
            if estimated_input_tokens > 0 or estimated_output_tokens > 0:
                if not self.check_token_limits(
                    ...
                    logger.warning(
                        "Proceeding despite token limit concerns (rate limiter continues with request limiting)"
                    )
    ...
    def get_current_usage(self):
        ...
        return {
            ...
            "requests_usage_percentage": (
                len(active_requests) / self.max_requests_per_minute
            )
            * 100,
            "input_tokens_used": current_input_tokens,
            "input_tokens_limit": self.max_input_tokens_per_minute,
            "input_tokens_usage_percentage": (
                current_input_tokens / self.max_input_tokens_per_minute
            )
            * 100,
            "output_tokens_used": current_output_tokens,
            "output_tokens_limit": self.max_output_tokens_per_minute,
            "output_tokens_usage_percentage": (
                current_output_tokens / self.max_output_tokens_per_minute
            )
            * 100,
            "active_workers": self.active_workers,
            "max_workers": self.max_workers,
            "workers_usage_percentage": (
                (self.active_workers / self.max_workers) * 100
