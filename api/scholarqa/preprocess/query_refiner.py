...
class RefinedElements(BaseModel):
    ...
    setting: Optional[str] = Field(default=None, description="Geographic region or population context")
    climate_factor: Optional[str] = Field(default=None, description="Climate-related factor, exposure, or intervention")
    health_outcome: Optional[str] = Field(default=None, description="Health outcomes of interest")
    temporal_scope: Optional[str] = Field(default=None, description="Temporal scope: immediate, short-term, medium-term, or long-term effects or actions")


class QueryRefinementAnalysis(BaseModel):
    """Analysis of query completeness and missing elements"""
    is_setting_clear: bool = Field(description="Whether geographic/population context is clear")
    ...
    needs_clarification: bool = Field(description="Whether any element needs clarification")


class InteractiveRefinementStep(BaseModel):
    """Single step in the refinement process"""
    element_type: str = Field(description="Type of element being refined: setting, climate_factor, health_outcome, or temporal_scope")
    prompt: str = Field(description="The prompt to show to the user")
    is_suggestion: bool = Field(default=False, description="Whether this is a suggestion or initial clarification")
...
class QueryRefinementResult(BaseModel):
    ...
    conversation_history: List[Tuple[str, str]] = Field(default_factory=list, description="List of (role, message) tuples")
    interactive_steps: List[InteractiveRefinementStep] = Field(default_factory=list, description="Steps for interactive refinement")
    needs_interaction: bool = Field(description="Whether the query needs interactive refinement")
...
def check_element_clarity(
    ...
    **llm_kwargs
    ...
    try:
        ...
        is_clear = result.content.strip().startswith(expected_responses[element_type])
...
def looks_broad(user_answer: str) -> bool:
    ...
    """
    broad_indicators = [
        "global", "worldwide", "all countries", "all regions", "general", "overall", "broad", 
        "climate change", "health outcomes", "population", "people in general",
        "displaced communities", "communities", "refugees"  # Added common broad population terms
    ]
    ...
    return any(indicator in answer_lower for indicator in broad_indicators)
...
def assess_response_specificity(element_type: str, user_answer: str, llm_model: str, **llm_kwargs) -> bool:
    """
    ...
    """
    element_guidance = {
        "setting": "population/setting specificity (geographic location, specific population groups)",
        "climate_factor": "climate factor specificity (specific climate exposures vs. general climate change)",
        "health_outcome": "health outcome specificity (specific diseases/conditions vs. general health)",
        "temporal_scope": "temporal scope specificity (defined timeframes vs. vague time references)"
    }
...
"""
    ...
    try:
        ...
        return looks_broad(user_answer)


def create_clarification_prompt(element_type: str, user_answer: Optional[str] = None) -> str:
    """
...
def analyze_query_completeness(
    ...
    **llm_kwargs
    ...
    setting_clear, setting_completion = check_element_clarity(query, "setting", llm_model, **llm_kwargs)
    completions.append(setting_completion)
    
    climate_clear, climate_completion = check_element_clarity(query, "climate_factor", llm_model, **llm_kwargs)
    completions.append(climate_completion)
    
    health_outcome_clear, health_completion = check_element_clarity(query, "health_outcome", llm_model, **llm_kwargs)
    completions.append(health_completion)
    
    temporal_clear, temporal_completion = check_element_clarity(query, "temporal_scope", llm_model, **llm_kwargs)
    completions.append(temporal_completion)
    
    needs_clarification = not (setting_clear and climate_clear and health_outcome_clear and temporal_clear)
...
def create_interactive_refinement_steps(
    ...
    if not analysis.is_setting_clear:
        steps.append(InteractiveRefinementStep(
...
def refine_query_with_conversation(
    ...
    **llm_kwargs
    ...
    try:
        result = llm_completion(
            user_prompt=PROMPT_QUESTION_REFORMULATION.format(conversation_history=formatted_history),
            system_prompt=SYSTEM_PROMPT_QUERY_REFINEMENT,
            model=llm_model,
            temperature=0.1,
            **llm_kwargs
...
def refine_research_question_interactive(
    ...
    **llm_kwargs
    ...
    setting_clear, setting_completion = check_element_clarity(user_question, "setting", llm_model, **llm_kwargs)
    completions.append(setting_completion)
    ...
    climate_clear, climate_completion = check_element_clarity(user_question, "climate_factor", llm_model, **llm_kwargs)
    completions.append(climate_completion)
    ...
    if not climate_clear:
        ...
        if climate_responses:
            ...
            conversation_history.append(("assistant", PROMPT_CLIMATE_FACTOR_CLARIFICATION))
            conversation_history.append(("user", user_answer))
        ...
        refined_elements.climate_factor = extract_element_from_query(user_question, "climate_factor")
    
    # STEP 3: health outcome factor check
    health_outcome_clear, health_completion = check_element_clarity(user_question, "health_outcome", llm_model, **llm_kwargs)
    completions.append(health_completion)
    ...
    if not health_outcome_clear:
        ...
        if health_responses:
            ...
            conversation_history.append(("assistant", PROMPT_HEALTH_OUTCOME_CLARIFICATION))
            conversation_history.append(("user", user_answer))
        ...
        refined_elements.health_outcome = extract_element_from_query(user_question, "health_outcome")
...
def run_query_refinement_step(
    ...
    **llm_kwargs
    ...
    # Determine what clarification is needed based on analysis and current user responses
    next_step = determine_next_clarification_step(analysis, user_responses)
    
    # If user responses are provided, process the current step
    if user_responses and next_step:
        # Process the current user response and determine if we need a follow-up
        processed_step = process_user_response_for_step(next_step, user_responses, llm_model, **llm_kwargs)
        if processed_step:
            conversation_history.extend(processed_step["conversation"])
            if processed_step["element_value"]:
                setattr(refined_elements, next_step["element_type"], processed_step["element_value"])
            ...
            if processed_step.get("needs_followup"):
                ...
                next_step = determine_next_clarification_step(analysis, user_responses, skip_element=next_step["element_type"])
    
    # If we have completed the interactive refinement, generate the final query
    if user_responses and not next_step:
        ...
        needs_interaction = len(create_interactive_refinement_steps(query, analysis)) > 0
        if analysis.is_setting_clear and analysis.is_climate_factor_clear and analysis.is_health_outcome_clear and analysis.is_temporal_scope_clear:
            # Extract elements from the original query
            refined_elements.setting = extract_element_from_query(query, "setting") or "global population"
            refined_elements.climate_factor = extract_element_from_query(query, "climate_factor") or "climate change in general"
            refined_elements.health_outcome = extract_element_from_query(query, "health_outcome") or "all health outcomes"
            refined_elements.temporal_scope = extract_element_from_query(query, "temporal_scope") or "all timeframes"
    
    # Create interactive steps for the frontend
    if needs_interaction and next_step:
        interactive_steps = [InteractiveRefinementStep(
    ...
    **llm_kwargs
    ...
    if step.get("is_suggestion", False):
        ...
        if len(responses) >= 2:
            ...
            conversation.append(("assistant", create_clarification_prompt(element_type)))
            ...
            element_value = followup_answer if followup_answer.strip() and followup_answer != "[SKIP]" else initial_answer
        else:
...
def get_next_refinement_step(
    refinement_result: QueryRefinementResult,
    step_index: int = 0
